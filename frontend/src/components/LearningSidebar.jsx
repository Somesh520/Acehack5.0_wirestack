  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        >
          {React.cloneElement(icon, { size: compact ? 14 : 16, strokeWidth: 3 })}
        </div>
        <div className="min-w-0">
          <div className={cn("font-black text-black/50 uppercase leading-none mb-1.5 truncate", compact ? "text-[8px] tracking-[0.12em]" : "text-[9px] tracking-[0.16em]")}>
            {label}
          </div>
          <div className={cn("font-black text-black leading-none truncate", compact ? "text-base" : "text-lg")}>{value}</div>
        </div>
      </div>
    </motion.div>
  );
...
function NavItem({ icon, label, isActive, onClick, badge }) {
  return (
    <motion.button
      ...
      onClick={onClick}
      className={cn(
        "w-full flex items-center gap-3 px-3.5 py-3 rounded-xl border-[3px] border-black font-black text-left transition-all",
        isActive
          ? "bg-[#FFE145] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
          : "bg-white hover:bg-gray-100 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
      )}
    >
      <div className="flex-shrink-0 w-5 h-5 flex items-center justify-center">{React.cloneElement(icon, { size: 20, strokeWidth: 2.5 })}</div>
      <span className="flex-1 text-[13px] uppercase tracking-tight leading-tight">{label}</span>
      {badge && (
        <div className="bg-[#FF3EA5] text-white border-2 border-black font-black px-2.5 text-[9px] rounded-full flex-shrink-0">
          {badge}
        </div>
      )}
      <ChevronRight size={18} strokeWidth={3} className="flex-shrink-0 text-black/60" />
    </motion.button>
  );
}
...
export default function LearningSidebar({
  ...
  const menuItems = [
    ...
    { id: "debug_lab", icon: <Bug />, label: "Issue Lab" },
  ...
  return (
    <aside className={cn(
      "w-[320px] md:w-[328px] h-dvh min-h-dvh max-h-dvh bg-gradient-to-br from-[#FFE1FF] via-[#F0F7FF] to-[#E1FFFF] border-r-8 border-black shrink-0 sticky top-0 font-mono z-50 overflow-hidden flex flex-col",
      isCompact ? "p-2.5 gap-2.5" : "p-3.5 md:p-4 gap-3"
    )}>
      ...
      {/* Brand & Identity Header - Click to View Profile */}
      <motion.button
        whileHover={{ y: -1 }}
        onClick={() => onNavigate('profile')}
        className="relative p-3 rounded-2xl border-[3px] border-black bg-white shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] flex-shrink-0 w-full text-left hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all cursor-pointer"
      >
          ...
          <div className="relative w-14 h-14 shrink-0">
             ...
             <div className="relative w-full h-full rounded-full border-4 border-black bg-[#FF3EA5] flex items-center justify-center overflow-hidden">
                {user?.profile_picture ? (
                    ...
                    <UserIcon size={24} className="text-white" strokeWidth={4} />
                )}
             </div>
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-[16px] font-black text-black leading-tight truncate">{displayName}</h3>
            <p className="text-[8px] font-black text-[#FF3EA5] uppercase tracking-[0.18em] flex items-center gap-1 mt-0.5">
              <Activity size={10} strokeWidth={4} /> STATUS::ACTIVE
            </p>
            <p className="text-[8px] font-black text-black/45 mt-1.5 uppercase tracking-wide truncate">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden pr-1 space-y-3 scrollbar-thin">
        ...
        <SectionBlock title="Project_Dock">
          ...
          <div className="max-h-24 overflow-auto space-y-1.5 pr-1">
            {projects.length === 0 ? (
          ...
          <AnimatePresence>
            {showStacks && (
              <motion.div
                ...
                className="flex flex-col gap-1.5 overflow-auto px-0.5 pt-1.5 max-h-40"
              >
      ...
      {!isUltraCompact && (
        <motion.button
          ...
          className={cn(
            "w-full rounded-2xl border-[3px] border-black bg-gradient-to-r from-[#3EFFB2] to-[#2ecc71] shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] hover:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] transition-all flex items-center justify-center gap-2 group flex-shrink-0",
